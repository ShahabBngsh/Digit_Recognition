...
def Hypothesis(X, params):
   ...
   z1 = np.matmul(a0, params["W1"].T) # a0*theta1'
   a1 = ReLU(z1) + params["b1"].T
   a1 = np.c_[np.ones(a0.shape[0]), a1]
   z2 = np.matmul(a1, params["W2"].T)
   a2 = ReLU(z2) + params["b2"].T
...
# %%
# print(J)
params, AllCost = NN_Model(X, one_hot, 100, 0.001)
prd = Predict(X, params)
print(np.mean(prd == y))
# %%

plt.scatter(np.arange(100), AllCost)
plt.show()
